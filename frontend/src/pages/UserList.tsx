...
function UserDetailsModal({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  return <Modal isOpen={Boolean(id)} onClose={onClose} title="Detalhes do Usuário">{isLoading || !user ? <Skeleton className="h-24 w-full" /> : <div className="space-y-4"><div className="flex items-center gap-4"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-xl font-bold text-primary">{user.name.slice(0, 2).toUpperCase()}</div><div><h2 className="text-xl font-bold">{user.name}</h2></div></div><p className="break-all font-mono text-xs text-muted-foreground"><span className="font-sans font-semibold text-foreground">UUID: </span>{user.id}</p><p className="flex items-center gap-2"><Mail className="h-4 w-4" />{user.email}</p><p className="flex items-center gap-2"><Calendar className="h-4 w-4" />Criado em {new Date(user.createdAt).toLocaleString('pt-BR')}</p></div>}</Modal>;
}
