...
export function UserList() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-4"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={searchValue} onChange={(event) => { setSearchValue(event.target.value); setPage(1); }} placeholder="Pesquisar por nome ou e-mail..." className="h-10 w-full rounded-xl border bg-background pl-10 pr-4 text-sm outline-none focus:ring-2 focus:ring-ring" /></div></CardContent></Card>
...
function UserDetailsModal({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  return <Modal isOpen={Boolean(id)} onClose={onClose} title="Detalhes do Usuário">{isLoading || !user ? <Skeleton className="h-24 w-full" /> : <div className="space-y-4"><div className="flex items-center gap-4"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-xl font-bold text-primary">{user.name.slice(0, 2).toUpperCase()}</div><div><h2 className="text-xl font-bold">{user.name}</h2><p className="text-xs text-muted-foreground">ID: {user.id}</p></div></div><p className="flex items-center gap-2"><Mail className="h-4 w-4" />{user.email}</p><p className="flex items-center gap-2"><Calendar className="h-4 w-4" />Criado em {new Date(user.createdAt).toLocaleString('pt-BR')}</p></div>}</Modal>;
}
