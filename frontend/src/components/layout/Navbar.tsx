...
export function Navbar({ onMenuToggle, theme, onThemeToggle }: { onMenuToggle: () => void; theme: 'light' | 'dark'; onThemeToggle: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[92px] items-center justify-between border-b border-[#ebeff6] bg-white px-8">
      <div className="flex items-center gap-6">
        ...
        <h1 className="text-[26px] font-semibold tracking-[-0.03em] text-[#181d27]">{title}</h1>
      </div>

      <div className="flex items-center gap-7">
        <div className="relative" ref={panelRef}>
          <button onClick={() => setOpen((current) => !current)} className="relative text-[#3b4457]">
          ...
          {open ? (
            <div className="absolute right-0 top-12 z-30 w-[380px] rounded-[22px] border border-[#e8edf5] bg-white p-4 shadow-[0_22px_60px_rgba(92,113,146,0.18)]">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-[17px] font-semibold text-[#181d27]">Notificações</div>
                  <div className="mt-1 text-[13px] text-[#748099]">
              ...
              <div className="mt-4 max-h-[420px] space-y-3 overflow-y-auto pr-1">
                {isLoading ? (
                  Array.from({ length: 3 }).map((_, index) => (
                    <div key={index} className="rounded-2xl border border-[#edf1f7] px-4 py-4">
                  ...
                  visibleNotifications.map((notification) => (
                    <Link
                      ...
                      className={`block rounded-2xl border px-4 py-4 transition ${
                        isRead(notification.id) ? 'border-[#edf1f7] bg-white' : 'border-[#e3d7ff] bg-[#faf7ff]'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          <div className="text-[14px] font-semibold text-[#1f2738]">{notification.title}</div>
                          <div className="mt-1 text-[13px] leading-5 text-[#606b82]">{notification.description}</div>
                        </div>
                        {!isRead(notification.id) ? <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#6c3df1]" /> : null}
                      </div>
                      <div className="mt-3 text-[12px] text-[#8b95aa]">{formatNotificationDate(notification.timestamp)}</div>
                    </Link>
                  ))
                ) : (
                  <div className="rounded-2xl border border-[#edf1f7] px-4 py-8 text-center text-[14px] text-[#748099]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 text-[15px] text-[#181d27]">
